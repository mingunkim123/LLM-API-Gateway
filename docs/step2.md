...
## 여기서 중요한 HTTP 포인트(개념 보충)
- **Header(...)**: 이 부분이 FastAPI의 마법입니다. 클라이언트(사용자)가 보낸 요청의 HTTP 헤더 중에서 `x-api-key`라는 이름을 가진 값을 자동으로 찾아서 `x_api_key` 변수에 넣어줍니다.
- **... (Ellipsis)**: 이 값은 필수라는 뜻입니다. 만약 헤더에 이 키가 없으면, 아래 로직을 실행하기도 전에 FastAPI가 알아서 "헤더에 키가 없어요!"라고 응답을 잘라버립니다.
- **Depends(verify_api_key)**: FastAPI의 핵심인 '의존성 주입' 기능입니다. `secure_chat` 라우터가 실행되기 전에 무조건 `verify_api_key` 함수를 먼저 통과하도록 강제하는 '검문소' 역할을 합니다. 만약 인증에 실패하면 라우터 내부의 코드는 한 줄도 실행되지 않습니다.
- **tenant_name: str = ...**: `verify_api_key` 함수가 무사히 실행되고 나서 `return` 해준 값(예: "team-a")이 이 `tenant_name` 변수 안으로 쏙 들어옵니다. 덕분에 본 로직에서는 번거로운 확인 과정 없이 "아, A팀이 왔구나!" 하고 바로 알 수 있습니다.
